...
class M_General extends CI_Model{
    ...
    function get_Laporan($id){
        ...
        $all_tagihan = $this->db->query("SELECT s.nama_siswa AS name, j.nama_tagihan AS jenis_tagihan, j.nominal_tagihan AS nominal 
                                       ...
                                       JOIN siswa AS s ON s.nis = t.nis
        ...
        foreach($all_tagihan as $row) {
            ...
            if(strpos($jenis, 'SPP') !== false) {
                ...
                $tagihan_lainnya[] = (object)['keterangan' => $row->name . " (".$row->jenis_tagihan.")", 'nominal' => $row->nominal];
            }
        }

        $pemasukan = $this->db->query("SELECT nominal_pemasukan AS nominal, ket_pemasukan AS keterangan FROM pemasukan p WHERE DATE(tgl_pemasukan) = '$t' AND NOT EXISTS (SELECT 1 FROM tagihan_siswa ts WHERE ts.id_pemasukan = p.id_pemasukan)")->result();

        $gaji     = $this->db->query("SELECT u.nama_guru AS name, g.periode, (g.jam * g.nominal_gaji) as gaji FROM gaji AS g JOIN guru AS u ON g.NUPTK = u.NUPTK WHERE DATE(g.tgl_gaji) = '$t'")->result();
        ...
        return array(
            ...
            'spp' => $spp,
            'ujian' => [],
            'tagihan_lainnya' => $tagihan_lainnya,
            'tanggal' => $t
        );
    ...
    function get_Laporan_periode($awal, $akhir){
        ...
        $pemasukan = $this->db->query("SELECT nominal_pemasukan AS nominal, ket_pemasukan AS keterangan FROM pemasukan p WHERE DATE(tgl_pemasukan) >= '$t_awal' AND DATE(tgl_pemasukan) <= '$t_akhir' AND NOT EXISTS (SELECT 1 FROM tagihan_siswa ts WHERE ts.id_pemasukan = p.id_pemasukan)")->result();

        $gaji     = $this->db->query("SELECT u.nama_guru AS name, g.periode, (g.jam * g.nominal_gaji) as gaji FROM gaji AS g JOIN guru AS u ON g.NUPTK = u.NUPTK WHERE DATE(g.tgl_gaji) >= '$t_awal' AND DATE(g.tgl_gaji) <= '$t_akhir'")->result();
